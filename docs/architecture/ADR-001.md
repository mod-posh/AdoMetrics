...
1. The module’s primary architectural contract is a **canonical metric row** schema, produced from ADO build runs.
...
4. Schema evolution is handled by an **idempotent schema repair** step during import (`Repair-AdoMetricRowSchema`) that:
   ...
   - Ensures each row includes `schemaVersion`
   - Treats missing `schemaVersion` as version `1`
   - Rejects rows with `schemaVersion` greater than the module’s supported version
